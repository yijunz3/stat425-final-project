...
RMSE = (mean(sum((exp(actual_data) - exp(pred_data))^2)))^0.5
RMSE
RMSE_1 = (mean(sum((exp(actual_data_1) - exp(pred_data_1))^2)))^0.5
RMSE_1
...
pred_data = predict(mod_mixed, test_data)

actual_data = test_data$Deaths

RMSE_mixed = (mean(sum((actual_data - pred_data))^2))^0.5
RMSE_mixed
```
...
length(coef(mod_mixed))-1  #Actually the number of predictor should be 26, I have no idea where the 3 comes from
```
