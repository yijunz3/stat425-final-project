...
#The p-value is slightly greater than 0.05, but still a weak support. Thus,we try to apply a variance stabilizing transformation.
mod_linear_1 <- lm(log(Deaths)~.,data = modified_data)
summary(mod_linear_1)
...
bptest(mod_linear_1)
par(mfrow=c(2,2))
shapiro.test(resid(mod_linear_1))
plot(mod_linear_1,which=c(1,2))
```
# Variable Selection
...
```{r}
#We would reject the normality assumption, in order to reduce non-normality, we would like to apply box-cox transformation.
dim(modified_data)
```
